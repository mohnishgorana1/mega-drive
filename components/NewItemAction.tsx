...
export default function NewItemAction({ isCollapsed }: NewItemActionProps) {
    ...
    // Determine current folder ID from URL (e.g. /12345abcde -> 12345abcde)
    const currentFolderId = (pathName === "/" || pathName === "/favourites" || pathName === "/trash") 
        ? null 
        : pathName.split("/")[1]
    ...
    return (
        <div className="relative w-full" ref={dropdownRef}>
            ...
            >
                <Plus className={`h-5 w-5 shrink-0 transition-transform duration-300 ${isDropdownOpen ? "rotate-45" : ""}`} />
                {!isCollapsed && <span>New</span>}
            </button>

            {/* The Dropdown Menu */}
            {isDropdownOpen && (
                <div className={`absolute z-50 mt-2 flex w-48 flex-col rounded-xl border border-dark-400 bg-dark-200 py-2 shadow-2xl ${
